...
function tasksDoneCell(r: BodyworkJobListDto) {
...
function statusChip(status: BodyworkJobStatus) {
  const tone =
    status === "COMPLETED"
...
export function BodyworkPageContent() {
  ...
  const [jobs, setJobs] = useState<BodyworkJobListDto[]>([]);
  ...
  const [source, setSource] = useState<BodyworkJobSource>("CUSTOMER");
  ...
  const load = useCallback(async () => {
    ...
    const data = await apiFetch<BodyworkJobListDto[]>(`/bodywork-jobs${qs ? `?${qs}` : ""}`);
    setJobs(data);
  }, [isWorkView, workScope]);

  useEffect(() => {
    void load().catch((err) =>
      setError(err instanceof ApiError ? err.message : "Could not load bodywork jobs"),
    );
  }, [load]);
  ...
  async function createJob(e: FormEvent) {
    ...
    setError("");
    try {
      const job = await apiFetch<BodyworkJobListDto>("/bodywork-jobs", {
        method: "POST",
        body: JSON.stringify({
          customerId,
          source,
      ...
      setModalOpen(false);
      window.location.href = `/bodywork/${job.id}`;
    } catch (err) {
  ...
  const columns: TableColumn<BodyworkJobListDto>[] = useMemo(() => {
    if (isWorkView) {
      return [
        {
          id: "job",
          ...
          cell: (r) => (
            <div>
              <Link href={`/bodywork/${r.id}`} className="font-medium text-accent hover:underline">
                {r.jobNumber}
              </Link>
        ...
        },
        {
          id: "status",
          header: "Status",
          searchText: (r) => BODYWORK_JOB_STATUS_LABEL[r.status],
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold">{isWorkView ? "My bodywork jobs" : "Bodywork jobs"}</h1>
      ...
      <SearchableTable<BodyworkJobListDto>
        columns={columns}
        rows={jobs}
        getRowId={(r) => r.id}
        searchPlaceholder="Job number, customer, reg…"
        countLabel={(f, t) => `${f} of ${t} jobs`}
        emptyLabel={isWorkView ? "No jobs in your workshop queue" : "No bodywork jobs yet"}
      />

      {canWrite && (
        <Modal title="New bodywork job" open={modalOpen} onClose={() => setModalOpen(false)} size="lg" fixedHeight>
          <form onSubmit={(e) => void createJob(e)} className="flex min-h-0 flex-1 flex-col">
            ...
            <div>
              ...
              <select
                value={source}
                onChange={(e) => setSource(e.target.value as BodyworkJobSource)}
                className={inputClass}
              >
                {Object.entries(BODYWORK_JOB_SOURCE_LABEL).map(([k, label]) => (
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className={inputClass}
                placeholder="Overall damage description, insurance claim ref…"
              />
              <p className="mt-1 text-xs text-[var(--muted)]">
                Add each damaged panel as a separate task on the job card after creation.
              </p>
            </div>
            {canChargeVat ? (
