...
export function buildTaskLineInputs(
  job: Pick<JobVatRow, "vatEnabled" | "vatRatePercent">,
  tasks: JobTaskRow[],
...
export function buildExtraCostLineInputs(
  job: Pick<JobVatRow, "vatEnabled" | "vatRatePercent" | "paintMaterialCostNet" | "outsourcedCostNet">,
  canChargeVat: boolean,
): LineCalcInput[] {
  const vatRate = canChargeVat && job.vatEnabled ? Number(job.vatRatePercent) : 0;
  const lines: LineCalcInput[] = [];
  const paint = Number(job.paintMaterialCostNet);
  const outsourced = Number(job.outsourcedCostNet);
  ...
  if (outsourced > 0) {
    lines.push({
      lineType: InvoiceLineType.LABOUR,
      description: "Outsourced work",
      quantity: 1,
      unitPriceNet: outsourced,
      vatRatePercent: vatRate,
    });
  }
...
export function computeJobInvoiceTotals(
  ...
  const lineInputs = [
    ...
    ...buildExtraCostLineInputs(job, canChargeVat),
  ];
